...
def get_notes_of_beat(notes,beat,tick_per_beat):
    ...
    return list(filter(lambda n: n.start<=start_tick<=n.end or n.start<=end_tick<=n.end or start_tick<=n.start<=n.end<=end_tick, notes))
...
def octave_transpose(transpose_notes, tick_per_beat,threshold=10,center_threshold=5,minmaxrange=None,mode='normal',shiftbase=False,deletesparse=False):
    # octave transpose
            ...
            if note.pitch>centers[closest_center]:                
                ...
                if closest_center==0: 
                    ...
                    if transpose_notes[note_idx].pitch<max(range_per_beat[start_beat:end_beat+1,0]):#the tranposed note can never go beyond the skyline
                        ...
                        #-----------------------------remove sparse note----------
                        current_beat_notes = get_notes_of_beat(transpose_notes,current_beat,tick_per_beat)
                        current_beat_classified_notes,abs_dist = cluster_assignment(current_beat_notes,centers_per_beat,tick_per_beat)
                        if transpose_notes[note_idx].pitch - min(current_beat_classified_notes[0],key=lambda n:n.pitch).pitch > sparse_threshold:
                ...
                if closest_center==1:
                    ...
                    if transpose_notes[note_idx].pitch>min(range_per_beat[start_beat:end_beat+1,1]):#the tranposed note can never go beyond the skyline
                        ...
                        if max(current_beat_classified_notes[1],key=lambda n:n.pitch).pitch - transpose_notes[note_idx].pitch > sparse_threshold:
...
def postprocess(inpath,outpath,merge_threshold=0.00005,discrete_note_threshold=0.0005,merge=False):
    ...
    #print(f'before post-processing {org_notes} notes')
    #delete sparse
    notes, centers_per_beat,range_per_beat = octave_transpose(notes, tick_per_beat,0,1,range_per_beat,mode='minmax',deletesparse=True)
    #shiftbase
    ...
    print('output to:',outpath,'#notes:',cur_notes)